...
**Gmailの外部メールアカウント取得機能（Mail Fetcher）廃止**に伴う代替ソリューションとして開発されました。

> **補足**: Gmailには他のメールアカウント（プロバイダメール等）からPOP3経由でメールを自動取得する機能（通称「POP3吸出し機能」）がありましたが、この機能が廃止されました。このソフトウェアは、その代替として、プロバイダメールを自動的にGmail等へ転送します。

> **他の選択肢**: プロバイダによっては、メール転送機能をWebメール管理画面で提供している場合があります。ご利用のプロバイダで転送機能が利用可能であれば、それを使うこともできます。ただし、プロバイダの転送機能では対応できない以下のようなケースで、このソフトウェアが役立ちます：
> - プロバイダが転送機能を提供していない
> - 複数のプロバイダメールを統合管理したい
> - 転送条件を細かくカスタマイズしたい
> - メールの保持期間を自分で管理したい
...
このソフトウェアはオープンソースとして無料で提供していますが、導入サポートが必要な個人様・法人様には**有料サポートサービス**を提供しています。
